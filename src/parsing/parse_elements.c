...
int   process_elements(char *line, t_game *game)
{
    ...
    set_error(game, "ERR: unknown asset identifier\n");
    return 0;
}
...
int parse_texture(char *line, t_id id, t_game *game)
{
    if (!space_move(&line, id))
        return (set_error(game, "ERR: texture path missing or malformed\n"),0);
    trim_right(line);
    int len = ft_strlen(line);
    if (len < 4 || ft_strncmp(line + len - 4, ".xpm", 4) != 0)
        return (set_error(game, "ERR: texture must be .xpm path\n"),0);
    int fd = open(line, O_RDONLY);
    if (fd < 0)
        return (set_error(game, "ERR: cannot open texture file\n"),0);
    close(fd);
    set_flag(game, id);
    if (check_flag(game, CHECK_DUPLICATE))
        return (set_error(game, "ERR: duplicate asset identifier\n"),0);
...
{
    if (!space_move(&line, id))
        return (set_error(game, "ERR: color value missing or malformed\n"),0);
    char **rgb = ft_split(line, ',');
    if (!rgb || !rgb[0] || !rgb[1] || !rgb[2] || rgb[3])
        return (set_error(game, "ERR: color must be 3 comma-separated components\n"),free_split(&rgb), 0);
    int r = get_rgb_value(rgb[0]);
    int g = get_rgb_value(rgb[1]);
    int b = get_rgb_value(rgb[2]);
    free_split(&rgb);
    if (r < 0 || g < 0 || b < 0)
        return (set_error(game, "ERR: color components must be integers 0-255\n"),0);
